...
class ActionSequenceModel(nn.Module):
    ...
    def forward(self, obs: torch.Tensor) -> Tuple[torch.Tensor]:
        ...
        #######################################################
        #########   YOUR CODE HERE - 3-9 lines.    ############
        mean = torch.tanh(net_out[:, :self.segment_len * self.action_dim])
        log_std = torch.clamp(net_out[:, self.segment_len * self.action_dim:], LOGSTD_MIN, LOGSTD_MAX)
...
class DPO(ActionSequenceModel):
    ...
    def update(
        ...
        dist = self.distribution(obs)
        curr_log_prob_w = dist.log_prob(actions_w)
        curr_log_prob_l = dist.log_prob(actions_l)
        with torch.no_grad():
            ref_log_prob_w = ref_policy.distribution(obs).log_prob(actions_w)
            ref_log_prob_l = ref_policy.distribution(obs).log_prob(actions_l)
        loss = torch.logsigmoid(self.beta * (curr_log_prob_w - ref_log_prob_w + curr_log_prob_l - ref_log_prob_l))
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
        #######################################################
